...
export async function GET(request: NextRequest) {
  return withApiRoute(request, "/api/escalation/operator/diagnostics", async (requestContext) => {
    ...
    const result = await context.services.escalation.diagnostics.getSummary({
      organizationId: context.actor.scope.organizationId,
      now: request.nextUrl.searchParams.get("now")?.trim() || undefined,
